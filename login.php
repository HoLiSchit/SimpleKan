...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_check($_POST['csrf_token'] ?? null)) {
        ...
        if ($user && (int)$user['locked_until'] > $now) {
            ...
            $pdo->prepare('UPDATE users SET failed_attempts = 0, locked_until = 0 WHERE id = :id')->execute([':id' => $user['id']]);
            ...
            if ($user) {
                ...
                $pdo->prepare('UPDATE users SET failed_attempts = :a, locked_until = :l WHERE id = :id')->execute([':a' => $attempts, ':l' => $lockUntil, ':id' => $user['id']]);
            }
...
<div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-8 w-full max-w-sm">
    ...
    <form method="post" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="username" required autofocus class="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-slate-400">
        </div>
        <div>
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Passwort</label>
            <input type="password" name="password" required class="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-slate-400">
        </div>
        <button type="submit" class="w-full bg-slate-800 dark:bg-slate-600 text-white rounded-lg py-2.5 font-medium hover:bg-slate-700 dark:hover:bg-slate-500 transition">Anmelden</button>
    </form>
</div>
</body>
</html>
