...
function ensure_schema(): void
{
    ...
    if (!table_has_column($pdo, 'cards', 'due_date')) {
        $pdo->exec('ALTER TABLE cards ADD COLUMN due_date TEXT');
    }
    if (!table_has_column($pdo, 'cards', 'priority')) {
        $pdo->exec('ALTER TABLE cards ADD COLUMN priority TEXT');
    }
    ...
    if ($colCount === 0) {
        $defaults = [
            ['neue_ideen', 'Neue Ideen', 'sky'],
            ['bugs', 'Bugs', 'rose'],
            ['todo', 'To Do', 'slate'],
            ['in_arbeit', 'In Arbeit', 'amber'],
            ['abgeschlossen', 'Abgeschlossen', 'emerald'],
            ['abgelehnt', 'Abgelehnte Ideen', 'neutral'],
        ];
        $stmt = $pdo->prepare('INSERT INTO board_columns (col_key, label, color, position) VALUES (:k, :l, :c, :p)');
        foreach ($defaults as $i => $d) {
            $stmt->execute([':k' => $d[0], ':l' => $d[1], ':c' => $d[2], ':p' => $i]);
        }
    }
}
