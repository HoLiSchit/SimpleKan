...
function normalize_priority(?string $p): ?string { if ($p === null || $p === '') return null; return in_array($p, PRIORITIES, true) ? $p : null; }
...
switch ("$method:$action") {
    case 'GET:list':
        ...
        $grouped = []; $flat = [];
        foreach ($rows as $row) {
            $card = ['id' => (int)$row['id'], 'title' => $row['title'], 'description' => $row['description'],
                'due_date' => $row['due_date'], 'priority' => $row['priority'], 'tag' => $row['tag'],
                'position' => (int)$row['position'], 'column' => $row['column_key']];
    ...
    case 'POST:reorder':
        $body = json_body(); $cols = $body['columns'] ?? [];
        ...
        try {
            ...
            $pdo->commit();
        } catch (Throwable $e) { $pdo->rollBack(); respond(['error' => 'Fehler beim Speichern der Reihenfolge.'], 500); }
        respond(['ok' => true]);
        break;
