...
{
    $stmt = $pdo->query('
        SELECT bc.col_key, bc.label, bc.color, bc.position, bc.wip_limit,
               COUNT(CASE WHEN c.archived = 0 THEN 1 END) AS card_count
        FROM board_columns bc
        LEFT JOIN cards c ON c.column_key = bc.col_key
        GROUP BY bc.col_key
...
switch ("$method:$action") {
    ...
    case 'POST:set_limit':
        $body = json_body();
        $key = (string)($body['key'] ?? '');
        $limit = max(0, (int)($body['wip_limit'] ?? 0));
    ...
    case 'POST:delete':
        ...
        try {
            if ($cardCount > 0 && $moveTo) {
                ...
                foreach ($cards->fetchAll(PDO::FETCH_ASSOC) as $row) {
                    $maxPos++;
            ...
            $pdo->rollBack();
            respond(['error' => 'Löschen fehlgeschlagen: ' . $e->getMessage()], 500);
        }
    ...
    case 'POST:reorder':
        ...
        try {
            ...
            foreach (array_values($order) as $pos => $key) {
                $stmt->execute([':p' => $pos, ':k' => (string)$key]);
            }
            $pdo->commit();
        } catch (Throwable $e) {
            $pdo->rollBack();
            respond(['error' => 'Sortierung fehlgeschlagen.'], 500);
        }
