...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($alreadyInstalled) {
        ...
        if ($username === '' || strlen($username) < 3) {
            ...
            db()->prepare('INSERT INTO users (username, password_hash) VALUES (:u, :p)')->execute([':u' => $username, ':p' => $hash]);
            $success = true;
        }
    }
}
    ...
    <?php else: ?>
        ...
        <form method="post" class="space-y-4">
            <div>
                ...
                <input type="text" name="username" required minlength="3" class="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-slate-400">
            </div>
            <div>
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Passwort (min. 8 Zeichen)</label>
                <input type="password" name="password" required minlength="8" class="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-slate-400">
            </div>
            <div>
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Passwort bestätigen</label>
                <input type="password" name="password_confirm" required minlength="8" class="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-slate-400">
            </div>
            <button type="submit" class="w-full bg-slate-800 dark:bg-slate-600 text-white rounded-lg py-2.5 font-medium hover:bg-slate-700 dark:hover:bg-slate-500 transition">Account erstellen</button>
        </form>
    <?php endif; ?>
</div>
</body>
